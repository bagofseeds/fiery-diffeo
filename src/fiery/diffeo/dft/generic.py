...
def kerdtn(
    ...
    if nbatch == 2:
        ...
        for d, m in enumerate(sym):
            ...
            for dd in range(d):
                kernel[d, dd] = dtn(
                    y[d, dd], obound, dim=dims, inverse=inverse
                )
...
def dtshift(
    ...
    slicer = [
        slice(int(floor(s / 2)), None) if is_sym else slice(None)
        for s, is_sym in zip(shape, sym)
    ]
    if nbatch == 2:
        slicer0 = [
            slice(int(floor(s / 2)), -1) if is_sym else slice(None)
            for s, is_sym in zip(shape, sym)
        ]
        slicer1 = [
            slice(int(floor(s / 2)) + 1, None) if is_sym else slice(None)
            for s, is_sym in zip(shape, sym)
        ]
