...
class DCT(torch.autograd.Function):
    ...
    def backward(ctx, x):
        ...
        x = from_cupy(
            F.idctn(to_cupy(x), type=ctx.type, axes=ctx.dim, norm=norm)
        )
...
class IDCT(torch.autograd.Function):
    ...
    def backward(ctx, x):
        norm = flipnorm[ctx.norm]
        x = from_cupy(
            F.dctn(to_cupy(x), type=ctx.type, axes=ctx.dim, norm=norm)
        )
        return x, None, None, None
...
class DST(torch.autograd.Function):
    ...
    def backward(ctx, x):
        norm = flipnorm[ctx.norm]
        x = from_cupy(
            F.idstn(to_cupy(x), type=ctx.type, axes=ctx.dim, norm=norm)
        )
        return x, None, None, None
...
class IDST(torch.autograd.Function):
    ...
    def backward(ctx, x):
        norm = flipnorm[ctx.norm]
        x = from_cupy(
            F.dstn(to_cupy(x), type=ctx.type, axes=ctx.dim, norm=norm)
        )
        return x, None, None, None
