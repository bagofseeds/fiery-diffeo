...
from fiery.diffeo.backends import default_backend
from fiery.diffeo.flows import compose
from fiery.diffeo.shoot import default_metric, shoot
from fiery.diffeo.svf import bch, exp
...
class Shoot(nn.Module):
    ...
    def __init__(
        self, metric=default_metric, steps=None, fast=True, backend=None
    ):
        """
        Parameters
        ----------
    ...
    def forward(self, v):
        v = v.movedim(1, -1)
        phi, _ = shoot(
            v, self.metric, self.steps, self.fast, backend=self.backend
        )
