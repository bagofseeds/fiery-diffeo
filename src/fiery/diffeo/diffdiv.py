...
from fiery.diffeo.utils import ensure_list, make_vector, same_storage
...
def diff1d(x, order=1, dim=-1, voxel_size=1, side='c', bound='dct2', out=None):
    ...
    def subto(x, y, out):
        """Smart sub"""
        if getattr(x, 'requires_grad', False) or getattr(
    ...
    if order == 1:
        ...
        diff = diff1d(
            x, order=1, dim=0, voxel_size=voxel_size, side=side, bound=bound
        )
        diff = diff1d(
            diff,
            order=order - 1,
...
def div(x, order=1, dim=-1, voxel_size=1, side='f', bound='dct2', out=None):
    ...
    if has_last and x.shape[-1] != nb_dim:
        raise ValueError(
            'Last dimension of `x` should be {} but got {}'.format(
                nb_dim, x.shape[-1]
            )
        )
    if not has_last:
