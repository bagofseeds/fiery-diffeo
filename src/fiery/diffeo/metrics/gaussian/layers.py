...
from fiery.diffeo.utils import cartesian_grid, make_vector
...
class Gaussian(Metric):
    ...
    def __init__(
        ...
        if getattr(self.fwhm, 'requires_grad', False) and cache:
            raise ValueError(
                'Cannot use both `cache=True` and a learnable fwhm'
            )
        self._metric_kernel = {}
    ...
    def greens_kernel(self, x, factor=True):
        ...
        if kernel is None:
            ...
            kernel = torch.stack(
                cartesian_grid(shape, dtype=x.dtype, device=x.device), -1
            )
            kernel -= (
                torch.as_tensor(shape, dtype=x.dtype, device=x.device)
                .sub_(1)
                .div_(2)
            )
            kernel *= voxel_size
    ...
    def greens_fourier(self, x, factor=True):
        ...
        if kernel is None:
            ...
            ft = FrequencyTransform(
                len(shape), self.bound, norm='ortho'
            ).forward_kernel
            kernel = ft(kernel)
