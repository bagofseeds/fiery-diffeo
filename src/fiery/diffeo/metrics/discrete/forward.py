...
from fiery.diffeo.diffdiv import diff, diff1d, div, div1d
from fiery.diffeo.utils import ensure_list, make_vector
...
def lame_shear(grid, voxel_size=1, bound='circulant'):
    ...
    if has_sliding(bound):
        bound = [
            [
                ('dst2' if d == dd else 'dct2') if b[0] == 's' else bound
        ...
        for j in range(i, ndim):
            for side_i in ('f', 'b'):
                ...
                if i == j:
                    # diagonal elements
                    mom[..., i].add_(
                        div1d(diff_ij, **opt_ij, out=buf2), alpha=0.5
                    )
                else:
                    ...
                    for side_j in ('f', 'b'):
                        ...
                        mom[..., j].add_(
                            div1d(diff_ji, **opt_ji, out=buf3), alpha=0.25
                        )
                        mom[..., i].add_(
                            div1d(diff_ji, **opt_ij, out=buf3), alpha=0.25
                        )
                    del x_j
        del x_i
    del grid
...
def lame_div(grid, voxel_size=1, bound='circulant'):
    """Precision matrix for the Linear-Elastic divergence component.
