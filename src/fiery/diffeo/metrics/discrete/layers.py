...
from fiery.diffeo.linalg import batchinv
from fiery.diffeo.metrics.base import Metric

from . import forward, kernels
...
class Mixture(Metric):
    ...
    def __init__(
        ...
        learnable_factor = learnable_factor or (
            isinstance(learnable, bool) and learnable
        )
        if not learnable_components:
            learnable_absolute = (
                isinstance(learnable, str) and 'absolute' in learnable
            )
            learnable_membrane = (
                isinstance(learnable, str) and 'membrane' in learnable
            )
            learnable_bending = (
                isinstance(learnable, str) and 'bending' in learnable
            )
            learnable_shears = (
                isinstance(learnable, str) and 'shears' in learnable
            )
        ...
        if learnable_absolute:
            self.absolute = nn.Parameter(
                torch.as_tensor(absolute), requires_grad=True
            )
        else:
            self.absolute = absolute
        if learnable_membrane:
            self.membrane = nn.Parameter(
                torch.as_tensor(membrane), requires_grad=True
            )
        else:
            self.membrane = membrane
        if learnable_bending:
            self.bending = nn.Parameter(
                torch.as_tensor(bending), requires_grad=True
            )
        else:
            self.bending = bending
        if learnable_shears:
            self.lame_shears = nn.Parameter(
                torch.as_tensor(lame_shears), requires_grad=True
            )
        else:
            self.lame_shears = lame_shears
        if learnable_div:
            self.lame_div = nn.Parameter(
                torch.as_tensor(lame_div), requires_grad=True
            )
        else:
            self.lame_div = lame_div
        if (
            getattr(self.absolute, 'requires_grad', False)
            or getattr(self.membrane, 'requires_grad', False)
            or getattr(self.bending, 'requires_grad', False)
            or getattr(self.lame_shears, 'requires_grad', False)
            or getattr(self.lame_div, 'requires_grad', False)
        ):
            raise ValueError(
                'Cannot use both `cache=True` and learnable components'
            )
        self._metric_kernel = {}
    ...
    def metric_fourier(self, x, factor=True):
        ...
        if kernel is None:
            ...
            patch = tuple(
                slice(
                    int(floor(s / 2)) - k // 2, int(floor(s / 2)) + k // 2 + 1
                )
