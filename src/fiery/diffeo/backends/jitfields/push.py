import torch
from jitfields.pushpull import count as _count
from jitfields.pushpull import push as _push

from fiery.diffeo.bounds import bound2dft
from fiery.diffeo.flows import add_identity
...
def push(image, flow, shape=None, bound='dct2', has_identity=False):
    ...
    if bound == 'sliding':
        ...
        return _push(
            image, flow, shape, bound=bound, order=1, extrapolate=True
        )
...
def count(flow, shape=None, bound='dct2', has_identity=False):
    ...
    if bound == 'sliding':
        ...
        for d in range(ndim):
            bound = ['dst2' if dd == d else 'dct2' for dd in range(ndim)]
            image.append(
                _count(flow, shape, bound=bound, order=1, extrapolate=True)
            )
        ...
        bound = list(map(lambda x: bound2dft.get(x, x), bound))
        return _count(
            flow, shape, bound=bound, order=1, extrapolate=True
        ).unsqueeze(-1)
