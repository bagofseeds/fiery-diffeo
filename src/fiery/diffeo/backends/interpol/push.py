import torch
from fiery.interpol import add_identity_grid as add_identity
from fiery.interpol import grid_count as _count
from fiery.interpol import grid_push as _push

from fiery.diffeo.bounds import bound2dft
from fiery.diffeo.utils import ensure_list
...
def push(image, flow, shape=None, bound='dct2', has_identity=False):
    ...
    if bound == 'sliding':
        ...
        image = _push(
            image, flow, shape, bound=bound, interpolation=1, extrapolate=True
        )
...
def count(flow, shape=None, bound='dct2', has_identity=False):
    ...
    if bound == 'sliding':
        ...
        for d in range(ndim):
            ...
            image.append(
                _count(
                    flow, shape, bound=bound, interpolation=1, extrapolate=True
                )
            )
