...
from fiery.diffeo.backends import interpol
from fiery.diffeo.flows import compose, jacobian
from fiery.diffeo.linalg import batchinv, batchmatvec
from fiery.diffeo.metrics.discrete import Mixture
...
def shoot(
    ...
    if not steps:
        ...
        with torch.no_grad():
            steps = (
                vel.square().sum(dim=-1).max().sqrt().floor().int().item() + 1
            )
    ...
    for i in range(1, abs(steps)):
        if fast:
            ...
            jac = batchinv(
                jacobian(idisp, bound=metric.bound, add_identity=True)
            )
        ...
        vel = vel.div_(steps)
        if verbose:
            energy = 0.5 * steps * (vel * mom).sum().item() / spatial.numel()
            print(
                f'{energy:6g}',
                end='\n' if not (i % 5) else ' ',
                flush=True,
