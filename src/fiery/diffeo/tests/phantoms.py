...
from fiery.diffeo.flows import identity
...
def letterc(shape, width=0.6, thickness=0.2, fwhm=0.01, **backend):
    ...
    innerc = circle(
        shape, width - thickness, fwhm=0, dtype=torch.bool, device=c.device
    )
    c[innerc] = 0
    thickness *= int(math.ceil(min(shape)))
    hole = (
        slice(
            int((shape[0] - thickness) // 2), int((shape[0] + thickness) // 2)
        ),
...
def smooth(x, fwhm):
    ...
    kernel -= (
        torch.as_tensor(shape, dtype=x.dtype, device=x.device).sub_(1).div_(2)
    )
